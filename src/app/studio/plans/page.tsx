  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div>
        ...
        <input required value={form.name} onChange={e => setForm({...form, name: e.target.value})} placeholder="e.g. Push Day..."
        ...
        <div className="max-h-48 overflow-y-auto space-y-1 border border-stone-700/50 rounded-xl p-2 bg-stone-800/30">
          {exercises.map((ex: any) => (
            <label key={ex.id} className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-all ${
              ...
              <input type="checkbox" checked={form.exerciseIds.includes(ex.id)} onChange={() => toggle(ex.id)} className="accent-orange-500 w-4 h-4" />
