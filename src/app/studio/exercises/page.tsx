  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div>
        ...
        <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Push Up"
...
export default function ExercisesPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    await api.delete(`/api/exercises/${id}`); load()
  }
        ...
        <div className="flex gap-1.5 bg-stone-900 border border-stone-700/50 rounded-xl p-1 relative">
          {['all', ...CATEGORIES].map(cat => (
            <button key={cat} onClick={() => setFilterCat(cat)}
              ...
              {filterCat === cat && (
                <motion.div layoutId="cat-pill" className="absolute inset-0 rounded-lg bg-fire-grad shadow-fire"
                  transition={{ type: 'spring', bounce: 0.25, duration: 0.4 }} style={{ zIndex: -1 }} />
              )}
              <span className={filterCat === cat ? 'text-white' : 'text-stone-500 hover:text-stone-300'}>{cat}</span>
            </button>
          ))}
        </div>
      </div>
              ...
              <motion.div key={ex.id} whileHover={{ y: -3 }}
                ...
                <div className="pl-2">
                  ...
                  <div className="flex gap-1.5 flex-wrap mb-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${TAG_COLORS[ex.category] || 'bg-stone-800 text-stone-400 border-stone-700'}`}>
                      {ex.category}
                    </span>
                    {ex.muscleGroup && (
