...
function getLast7Days() {
  return Array.from({ length: 7 }, (_, i) => {
    ...
    return { date: d.toISOString().split('T')[0], label: i === 6 ? 'Today' : d.toLocaleDateString('en', { weekday: 'short' }), day: d.getDate() }
  })
}
...
export default function ChecklistPage() {
  ...
  const toggle = async (id: number) => { await api.patch(`/api/checklist/${id}`); loadChecklist() }
  const remove = async (id: number) => { await api.delete(`/api/checklist/${id}`); loadChecklist() }

  const addExercises = async () => {
    for (const exId of selectedEx) await api.post('/api/checklist', { exerciseId: exId, date: selectedDate })
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="font-display text-5xl tracking-wider leading-none"
          style={{ background: 'linear-gradient(135deg, #DC2626 0%, #EA580C 55%, #F59E0B 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
          DAILY CHECKLIST
        </h1>
        ...
        <div className="bg-stone-900 border border-stone-700/50 rounded-2xl p-4 mb-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              {pct === 100 && <Flame size={18} className="text-amber-500 animate-pulse" />}
              <span className="font-display text-xl tracking-wider text-white">{done}/{total} DONE</span>
            </div>
            <span className={`font-display text-2xl ${pct === 100 ? '' : 'text-ember-500'}`}
              style={pct === 100 ? { background: 'linear-gradient(135deg, #DC2626, #EA580C, #F59E0B)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' } : {}}>
              {pct}%
            </span>
          </div>
          <div className="h-2.5 bg-stone-800 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6 }} className="h-full bg-fire-grad rounded-full" />
          </div>
          {pct === 100 && <p className="text-center text-xs font-bold text-amber-500 mt-2">WORKOUT COMPLETE! CRUSHING IT!</p>}
        </div>
      )}
            ...
            {checklist.map((item) => (
              <motion.div key={item.id} exit={{ opacity: 0, x: 16 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className={`font-bold text-sm ${item.isCompleted ? 'line-through text-stone-500' : 'text-white'}`}>{item.exercise?.name}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${TAG_COLORS[item.exercise?.category] || 'bg-stone-800 text-stone-400 border-stone-700'}`}>
      ...
      <Modal open={showPlanModal} onClose={() => setShowPlanModal(false)} title="LOAD WORKOUT PLAN">
        <div className="space-y-2">
          ...
          {plans.length === 0 && <p className="text-center text-stone-500 py-4 text-sm">No plans yet.</p>}
        </div>
      </Modal>
    </>
  )
