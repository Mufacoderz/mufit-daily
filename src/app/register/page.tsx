  ...
  return (
    <div className="min-h-screen bg-stone-950 flex items-center justify-center px-6 py-12">
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-sm">
        ...
        <p className="text-stone-500 text-sm mb-8">Start building better habits today</p>
        {error && (
          <div className="bg-red-950/50 border border-red-800/50 text-red-400 text-sm font-semibold px-4 py-3 rounded-xl mb-5">⚠️ {error}</div>
        )}
        ...
        <p className="text-center text-sm text-stone-500 mt-6">
          Already have an account? <Link href="/login" className="text-ember-500 font-bold hover:text-ember-400">Log in</Link>
        </p>
      </motion.div>
    </div>
  )
